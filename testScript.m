...
if isempty(cache(h))
  [estimate, kmax, loglikelihoods, bandwidths, CI] = BandwidthEstimator.cvKernel(root, spikeTrain(1:nEpochSteps), range, false);
...
estimax = 0;
for ii = 1:4
  bandwidth = round([0.5 0.75 1.0 1.25]*kmax);
  bandwidth(mod(bandwidth, 2) == 0) = bandwidth(mod(bandwidth, 2) == 0) + 1;
